 ...
 * @author caf
 */
public class SriRecepcionProcessor extends QueueProcessorThread
{
    private Propiedades propiedades = null;
    
    public SriRecepcionProcessor(int poolSize, int maxPoolSize, long keepAliveTime, int ArrayQueueSize,Propiedades propiedades) {
            ...
            try 
            {
                if(getInData().getData().get(0).getData().size() > 0)
                {
                    do 
                    {
                        ...
                        addTaskToQueue(new SriRecepcionTask(queueDataProcessor,propiedades, this));
